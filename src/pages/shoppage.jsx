    ...
    const ShopPage = ({ shops }) => {
      ...
      return (
        <>
          <Navbar />
          <div className="coverImg flex flex-col justify-center items-center md:h-[500px] h-40 w-full bg-cover">
              ...
              <div className="grid md:grid-cols-3 grid-cols-1 gap-8">
                {currentShops.length > 0 ? (
                  currentShops.map((shop) => (
                    <div
                      key={shop.id}
                      className="product-card md:w-[20rem] w-[18rem] bg-slate-50 shadow-slate-200 shadow-lg border-white border-2 rounded-md p-2 m-2 flex flex-col justify-between"
                    >
                      <Link to={`/shop/${shop.id}`}>
                        <img
                          ...
                          src={`http://192.168.1.11:1337${shop.attributes.productImage.data.attributes.url}`}
