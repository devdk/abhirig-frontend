...
const PriceFilter = ({ handleFilter }) => {
  ...
  return (
    <div className="filter-widget p-4 rounded-md">
      <h3 className="text-2xl uppercase font-semibold tracking-tighter mb-6">Filter by Price</h3>
      <hr className="my-4" />
      <div className="flex items-center space-x-4">
        ...
        <div className="flex-1 ">
          ...
          <Slider
            ...
            ref={sliderRef} 
            className="w-full mt-1 custom-slider"
          />
        </div>
        {/* Apply Button */}
        <button
          className="px-4 py-2 bg-lightGreen text-white rounded hover:scale-95 transition-all duration-300 hover:bg-lightGreen"
          onClick={applyFilter}
        >
          Apply
        </button>
        {/* Clear Button */}
        <button
          className="px-4 py-2 bg-gray-300 text-gray-700 rounded hover:bg-gray-200 transition-colors duration-300"
          onClick={clearFilter}
        >
          Reset
        </button>
