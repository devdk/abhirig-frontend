    ...
    const fetchProducts = async () => {
      try {
        const response = await fetch(
          "http://192.168.1.11:1337/api/shops?populate=*"
        );
        if (!response.ok) {
  ...
  return (
    <>
      <div className="container flex flex-col mx-auto py-8">
        <h1 class="mb-10 text-2xl font-extrabold leading-none text-center tracking-tight text-gray-900 md:text-5xl lg:text-6xl ">
          Discover our
              ...
              <div key={product.id} className="cardSlider mt-4">
                <div className="product-card flex flex-col gap-4 justify-center text-center items-center mx-auto p-4">
                  <img
                    ...
                    src={`http://192.168.1.11:1337${product.imageUrl}`}
                    alt={product.name}
                  />
                  <div className="pb-4">
                    <a
